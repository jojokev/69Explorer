  ...
  fromBooking.addEventListener("submit", (e) => {
    ...
    while (form) {
      ...
      auxInputs.forEach((input) => {
        if (
          input.name !== "travellers-restrictions-0" &&
          input.name !== "travellers-student-0"
        )
          arg += input.value + ", ";
      });
      auxCount++;
    ...
    peardata.append(
      "Informacion del tipo de habitacion",
      fromBooking.typeRoomInformation.value
    );
    peardata.append(
      "Ubicacion del hotel del pasajero",
      fromBooking.hotelLocation.value
    );
    peardata.append(
      "Numeros de emergencias",
      fromBooking.emergencyNumber.value
    );
...
if (input) {
  dateFromPHP = input.dataset.date;
}
...
const idFormFields = (form, id) => {
  ...
  inputClone.forEach((item) => {
    ...
    if (item.getAttribute("base-name") === "travellers-name-")
      item.value = null;
    if (item.getAttribute("base-name") === "travellers-email-")
      item.value = null;
    if (item.getAttribute("base-name") === "travellers-country-")
      item.value = null;
    if (item.getAttribute("base-name") === "travellers-doc-") item.value = null;
    if (item.getAttribute("base-name") === "travellers-birth-")
      item.value = null;
  });
};
//actualizar valores
...
if (buttonItemTour) {
  buttonItemTour.forEach((item) => {
    item.addEventListener("click", () => {
      formItemTour.querySelector('input[name="tour"]').value =
        item.getAttribute("title-tour");
    });
  });
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if ($menuTitles) {
    ...
    if (languageMenu === "en")
      arrayMenuLang = [
        ...
        '<i class="bi bi-card-checklist"></i> Includes / Not Include',
        '<i class="bi bi-bookmark-star"></i> What to bring?',
        '<i class="bi bi-list-nested"></i> FAQs',
      ];
    if (languageMenu === "es")
      arrayMenuLang = [
        '<i class="bi bi-map"></i> Itinerario',
        '<i class="bi bi-card-checklist"></i> Incluye / No incluye',
        '<i class="bi bi-bookmark-star"></i> ¿Qué llevar?',
        '<i class="bi bi-list-nested"></i> FAQs',
      ];
    if (languageMenu === "zn")
      arrayMenuLang = [
        '<i class="bi bi-map"></i> 行程详情 行程',
        '<i class="bi bi-card-checklist"></i> 包含项目 / 不包含内容',
        '<i class="bi bi-bookmark-star"></i> 要带什么？',
        '<i class="bi bi-list-nested"></i> 常问问题',
      ];
    ...
    window.addEventListener("scroll", function () {
      if (ur && stickyElement) {
        if (
          $wrapperItinerario.getBoundingClientRect().top < 0 &&
          $wrapperItinerario.getBoundingClientRect().top >
            -1 * $wrapperItinerario.scrollHeight
        ) {
          // Si el top del elemento 'ur' está a -200px o menos
    ...
    $itemsMenuTitle.forEach((item) => {
      let aux =
        ...
        (window.innerWidth - item.scrollWidth) / 2;
      item.addEventListener("click", () => {
  ...
  const addActiveShow = () =>
    $navigationShows.forEach((el) => el.classList.add("show"));
  const removeActiveShow = () =>
    $navigationShows.forEach((el) => el.classList.remove("show"));
  ...
  const $itemsNavigation = document.querySelectorAll(
    "#items-navigation-tour > div"
  );
  ...
  $itemsNavigation.forEach((el, index) => {
    el.addEventListener("click", (e) => {
      window.scrollTo({
        top:
          $sectionNavigation[index].getBoundingClientRect().top +
          window.scrollY -
          60,
        behavior: "smooth",
      });
  ...
  const $buttonScrollTextTour = document.querySelectorAll(
    "[data-scrollButton]"
  );

  // Ocultar boton si la altura es menor al parametro scrollheight
  ...
  if ($scrollTextTour && $buttonScrollTextTour) {
    $scrollTextTour.forEach((text, index) => {
      if (
        !(
          text.dataset.scrollText ==
          $buttonScrollTextTour[index].dataset.scrollButton
        )
      )
        return;
      // Activar o desactivar la altura del texto
